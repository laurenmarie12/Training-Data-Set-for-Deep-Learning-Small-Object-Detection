...
metadata['info'] = {
  ...
  "version": "1.0",
  "year": 2023,
  "contributor": "Lauren Mangla, Shipeng Sun",
...
for i, geom_bounds in enumerate(geoms_by_bounds):
    ...
    for j, image_bounds in enumerate(images_by_bounds):
        geom_left, geom_bottom, geom_right, geom_top = geom_bounds
        if (geom_left >= image_bounds.left and geom_bottom >= image_bounds.bottom and geom_right <= image_bounds.right and geom_top <= image_bounds.top): # check if image contains geometry
            ...
            with rasterio.open("orthos/"+images_by_bounds[image_bounds]) as src:
                ...
                out_meta = src.meta
                out_meta.update({"driver": "GTiff", # tell rasterio to scale raster to 250x250 px when writing image
                 "height": 250,
                 "width": 250,
...
print(f"FAILED TO MATCH {len(mismatches)} items")
print("\n".join([f"Geometry {i} had {x} image matches" for i, x in enumerate(match_counts) if x != 1]))
